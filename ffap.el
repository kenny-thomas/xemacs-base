...
;;
;; XEmacs users might want to add this mouse-track binding:
...
(defconst ffap-version "1.9-gen <97/06/29 00:26:46 mic>"
  "The version of ffap: \"Major.Minor-Build <Timestamp>\"")

;; Allow compilation by users without Per Abrahamsen's new custom
;; library <http://www.dina.kvl.dk/~abraham/custom/>:
(eval-and-compile			; is this widely supported?
  (or (featurep 'custom) (load "custom" t))
  (if (fboundp 'custom-declare-variable) ; got it
      nil
...
;;; Compatibility:
;;
;; This versions of ffap is for recent versions of both Emacs and
;; XEmacs (including some light testing with both 20.* versions).
;; XEmacs users no longer need the file "ffap-xe.el".  Emacs 18 is no
;; longer supported, but try ffap-15.el.

(defvar ffap-xemacs (and (string-match "XEmacs" emacs-version) t)
  "Whether ffap thinks it is running under XEmacs.")

(cond
 (ffap-xemacs
  ;; XEmacs extents do *not* behave identically to Emacs overlays, but
  ;; these aliases should suffice for our purposes.
  (defalias 'ffap-make-overlay 'make-extent)
  (defalias 'ffap-delete-overlay 'delete-extent) ; not reusable
  (defalias 'ffap-move-overlay 'set-extent-endpoints)
  (defalias 'ffap-overlay-put 'set-extent-property) ; 'face
  (defalias 'ffap-find-face 'find-face)
  (defalias 'ffap-event-buffer 'event-buffer)
  (defun ffap-mouse-event nil current-mouse-event)
  )
 (t					; hope for Emacs 19.3*
  ...
  (let (dir)
	...
	(progn
	  (or (ffap-file-remote-p guess)
	      (setq guess
                    (apply 'abbreviate-file-name
			   (expand-file-name guess)
			   ;; HACK-HOMEDIR argument for XEmacs:
			   (if ffap-xemacs '(t)))
		    ))
...
(defvar ffap-highlight-overlay nil
  "Overlay \(or extent, in XEmacs\) used by `ffap-highlight'.")
...
Uses the face `ffap' if it is defined, or else `highlight'."
  (cond
   (remove
    ...
    (if ffap-xemacs (setq ffap-highlight-overlay nil)) ; extents not reusable
    )
...
(defvar ffap-menu-text-plist
  (cond
   ...
   (ffap-xemacs '(face bold highlight t)) ; keymap <map>
	    ...
	    (x-popup-menu
	     ...
	     (list "" (cons title
			    (mapcar (function (lambda (i) (cons (car i) i)))
				    alist))))))
     ;; XEmacs mouse:
     ((and (fboundp 'popup-menu-and-execute-in-window) ; see menubar.el
	   (ffap-mouse-event))
      (setq choice
	    (popup-menu-and-execute-in-window
	     (append
	      (list title "-")
	      (mapcar (lambda (i) (vector (car i) (list 'quote i) t)) alist))
	     (selected-window))))
...
(defvar ffap-mouse-track-clicks 3 "See `ffap-mouse-track-click'.")

(defun ffap-mouse-track-click (event clicks) ; mouse-track, XEmacs only
  "A possible entry for `mouse-track-click-hook'.
Tries ffap-at-mouse when click count reaches `ffap-mouse-track-clicks'.
To install, evaluate this:
\(add-hook 'mouse-track-click-hook 'ffap-mouse-track-click\)"
  ;; Problem: this is fairly slow as a mouse-track function.
  (and (eq clicks ffap-mouse-track-clicks) (ffap-at-mouse event)))
