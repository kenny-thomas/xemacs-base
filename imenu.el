...
;; Variable to which menu functions store the position.
(defvar imenu--menu-callback-variable nil)

;; Whether the menu creating functions should work for popup menus.
;; Internal crock.
(defvar imenu--popup-menu-p nil)
...
(defun imenu--create-menu-2 (list)
  (cond ((listp (cdr list))
	 (imenu--create-menu-1 (car list) (cdr list)))
	(t
	 (vector (car list)
		 (if imenu--popup-menu-p
		     `(setq imenu--menu-callback-variable (quote ,list))
		   `(imenu--menubar-select (quote ,list)))
		 t))))
...
Returns t for rescan and otherwise an element or subelement of INDEX-ALIST."
  ...
  (let* ((menu 	(imenu--split-menu index-alist
	 ...
	 (imenu--popup-menu-p t))
    (setq menu (imenu--create-menu-1 (car menu) 
				     (if (< 1 (length (cdr menu)))
					 (cdr menu)
				       (cdr (car (cdr menu))))))
    (setq imenu--menu-callback-variable nil)
    (popup-menu menu event)
    ;; Wait for menu to bail out.
    (let (event)
      (while (not (misc-user-event-p (setq event (next-event))))
	(dispatch-event event))
      (dispatch-event event))
    imenu--menu-callback-variable))
...
The returned value is of the form (INDEX-NAME . INDEX-POSITION)."
  (let (index-alist
    ...
    ;; really is selected.
    (and mouse-triggered
	 (not (misc-user-event-p last-command-event))
	 (let ((window (event-window last-command-event)))
	   (and window
